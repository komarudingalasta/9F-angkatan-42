...
async function loadRoleData(){
  ...
  if(isAdmin()){
    const [r,s,u,a,l,c,sub]=await Promise.all([
      db.collection("records").get(),db.collection("studentSummaries").get(),db.collection("users").get(),
      db.collection("attendance").get(),db.collection("leaveRequests").get(),db.collection("classRoster").get(),
      db.collection("subjects").get()
    ]);
    records=r.docs.map(d=>({id:d.id,...d.data()}));summaries=s.docs.map(d=>({id:d.id,...d.data()}));
    users=u.docs.map(d=>({uid:d.id,...d.data()}));attendance=a.docs.map(d=>({id:d.id,...d.data()}));
    leaveRequests=l.docs.map(d=>({id:d.id,...d.data()}));classRoster=c.docs.map(d=>({id:d.id,...d.data()}));
    subjects=sub.docs.map(d=>({id:d.id,...d.data()}));
  }else{
    const nis=String(profile.nis||"");
    const [r,s,a,l]=await Promise.all([
      db.collection("records").where("nis","==",nis).get(),
      db.collection("studentSummaries").where("nis","==",nis).get(),
      db.collection("attendance").where("nis","==",nis).get(),
      db.collection("leaveRequests").where("nis","==",nis).get()
    ]);
    records=r.docs.map(d=>({id:d.id,...d.data()}));summaries=s.docs.map(d=>({id:d.id,...d.data()}));
    attendance=a.docs.map(d=>({id:d.id,...d.data()}));leaveRequests=l.docs.map(d=>({id:d.id,...d.data()}));
  }
}
...
function renderAdminDashboard(){
  ...
  $("dashPresent").textContent=attendance.filter(x=>x.date===today()&&x.status==="Hadir").length;
...
function renderStudentAttendance(){
  ...
  $("studentLeaveHistory").innerHTML=[...leaveRequests].sort((x,y)=>String(y.createdAt||"").localeCompare(String(x.createdAt||""))).map(x=>`<div class="history-row"><b>${esc(x.type)} · ${esc(x.startDate)}</b><small>${esc(x.status||"Menunggu")}${x.note?` · ${esc(x.note)}`:""}</small></div>`).join("")||'<div class="muted">Belum ada pengajuan.</div>';
}
